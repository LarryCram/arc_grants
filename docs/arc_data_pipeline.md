...
Each grant carries between one and three Field of Research (FOR) codes drawn from the ANZSRC classification. The FOR hierarchy has three levels: a 2-digit division (~22 divisions), a 4-digit group (~254 groups), and a 6-digit field (not used in this project — ARC's own extract only ever carries the 4-digit group code). Grants awarded before the 2020 edition was introduced carry 2008 codes; grants since then carry 2020 codes. The two editions are structurally equivalent at the division and group levels but differ in their numeric codes, making direct cross-edition numeric comparison invalid without a concordance.

As of 2026-08-07, this 2008→2020 upgrade — and the separate FOR↔OpenAlex-topic harmonisation described below — is delegated to an external, audited package, `research_classification` (`pip install git+https://github.com/LarryCram/ResearchClassification.git`), via the thin wrapper `src/utils/for_resolve.py`. This replaced an earlier hand-built, in-repo concordance (`src/utils/lookup_for_topic.py`, fed by ~370 hand-typed mapping rows) that is no longer part of the codebase. The upgrade is still applied during Phase 1 via the same two DuckDB UDFs (`upgrade_for_code`, `upgrade_for_name`, now backed by `for_resolve.py`), so all downstream data still carries uniform ANZSRC 2020 codes.

ANZSRC FOR divisions and groups map bidirectionally and 1:1 to the OpenAlex/Scopus topic hierarchy used to classify research outputs: FOR divisions correspond to the 27 OAX fields (equivalent to Scopus subject categories), and FOR groups correspond to the 254 OAX subfields (equivalent to Scopus minor subjects). `research_classification` resolves this mapping directly and exactly (`for_resolve.oax_subfield_name()`), rather than approximating it via FOR-group-name token overlap as the pipeline previously did — that token-overlap approach was a stand-in for not having a real concordance available, and no longer applies to this cross-scheme step. A separate, unrelated concordance of 49 near-synonym FOR *name* pairs (Jaccard ≥ 0.5, `data_persisted/for_concordance.csv`) still bridges minor naming differences between ARC's own grant records for Splink blocking purposes — this is free-text fuzzy matching within ARC's own data, a different problem `research_classification` doesn't address (it only does exact code/label lookups).
...
6. **Manual overrides**: `data_persisted/manual_orcids.csv` (12 entries) injects verified ORCIDs; `data_persisted/manual_merges.csv` (11 entries) handles confirmed same-person pairs where automated steps failed.
